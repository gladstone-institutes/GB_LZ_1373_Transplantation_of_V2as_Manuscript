...
if [ ! -d "$source" ] || [ ! -f "$fasta_download" ] || [ ! -f "$gtf_download" ]; then
    echo "ERROR: Cannot find source files. Please download the source files before running this script."
    exit 1
fi

#unzip the downloaded source files
zcat "$fasta_download" > "$fasta_in"
zcat "$gtf_download" > "$gtf_in"
...
gtf_filtered="${source}/$(basename "$gtf_in").filtered"
