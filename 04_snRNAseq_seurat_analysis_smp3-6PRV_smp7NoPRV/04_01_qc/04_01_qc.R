...
indir <- paste0(basedir,"/02_cellranger_count/")
outdir <- paste0(basedir,"/04_snRNAseq_seurat_analysis_smp3-6PRV_smp7NoPRV")
...
for(i in 1:length(samples_list)){
  ...
  pdf(paste0(outdir,"/01_qc/",obj.name,"_post_qc_post_YChrGene_filter_genes_violinplot.pdf"))
  print(VlnPlot(this.obj, features = c("USP9Y", "UTY", "RPS4Y1")))
  dev.off()
  
  # Visualize QC metrics post filtering for human cells 
  pdf(paste0(outdir,"/01_qc/",obj.name,"_post_qc_post_YChrGene_filter_QCmetrics_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
  ...
  sink(file.path(outdir,"01_qc/UL_transcripts_post_qc_samples_3-7_grch38_HumanCells.txt"), append = TRUE)
  ...
  saveRDS(this.obj, file = paste0(outdir,"/01_qc/",obj.name,"_HumanCells.rds"))
  
}

#save the qc metadata
write.csv(cells_per_sample, 
          file = paste0(outdir,"/01_qc/qc_per_sample_metadata_samples_3-7_grch38_HumanCells.csv"), 
          row.names = FALSE)

#create a plot for cell counts before and after filtering for each sample
pdf(paste0(outdir, "/01_qc/","qc_cells_per_sample_plot_samples_3-7_grch38_HumanCells.pdf"),
    width = 20)
...
write.csv(as.data.frame(ribo_genes), row.names = F, 
          file = paste0(outdir,"/01_qc/ribo_genes_samples_3-7_grch38_HumanCells.csv"))
