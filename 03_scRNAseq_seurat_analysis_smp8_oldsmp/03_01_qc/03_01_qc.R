...
##              scRNAseq samples 8 and sample from previous run; perform QC
...
outdir <- paste0(basedir,"/03_scRNAseq_seurat_analysis_smp8_oldsmp")
...
samples_list <- c("LZ4087_08A_human","LZ4087_08B_human",
                  "LZ-3672_01","LZ-3672_02","LZ-3672_03","LZ-3672_04")
...
for(i in 1:length(samples_list)){
  ...
  #add metadata
  this.obj$sample_name <- sub("_human$", "", obj.name)
  ...
  sink(file.path(outdir,"01_qc/UL_transcripts_post_qc.txt"), append = TRUE)
...
write.csv(cells_per_sample, 
          file = paste0(outdir,"/01_qc/qc_per_sample_metadata.csv"), 
          row.names = FALSE)

#create a plot for cell counts before and after filtering for each sample
pdf(paste0(outdir, "/01_qc/","qc_cells_per_sample_plot.pdf"),
    width = 10)
