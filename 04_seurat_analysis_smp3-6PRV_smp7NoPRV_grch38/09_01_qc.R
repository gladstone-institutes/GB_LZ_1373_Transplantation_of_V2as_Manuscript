...
if(!dir.exists(file.path(outdir, "01_qc_no_cellbender_no_doubletfinder")))
  dir.create(file.path(outdir, "01_qc_no_cellbender_no_doubletfinder"), recursive = TRUE)
setwd(outdir)
...
for(i in 1:length(samples_list)){
  ...
  pdf(paste0(outdir,"/01_qc_no_cellbender_no_doubletfinder/",obj.name,"_pre_qc_plot.pdf"))
  ...
  pdf(paste0(outdir,"/01_qc_no_cellbender_no_doubletfinder/",obj.name,"_post_qc_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt"),
                ncol = 3))
  ...
  sink(file.path(outdir,"01_qc_no_cellbender_no_doubletfinder/UL_transcripts_post_qc_samples_3-7_grch38.txt"), append = TRUE)
  ...
  saveRDS(this.obj, file = paste0(outdir,"/01_qc_no_cellbender_no_doubletfinder/",obj.name,".rds"))
  
}

#save the qc metadata
write.csv(cells_per_sample, 
          file = paste0(outdir,"/01_qc_no_cellbender_no_doubletfinder/qc_per_sample_metadata_samples_3-7_grch38.csv"), 
          row.names = FALSE)

#create a plot for cell counts before and after filtering for each sample
pdf(paste0(outdir, "/01_qc_no_cellbender_no_doubletfinder/","qc_cells_per_sample_plot_samples_3-7_grch38.pdf"),
    width = 10)
...
writeLines(capture.output(sessionInfo()), 
           file.path(outdir, "sessionInfo_step01_qc_samples_3-7_grch38.txt"))
