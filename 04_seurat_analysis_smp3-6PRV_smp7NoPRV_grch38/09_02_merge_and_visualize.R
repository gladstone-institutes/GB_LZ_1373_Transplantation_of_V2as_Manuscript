...
data <- SCTransform(data, 
                    vst.flavor = "v2", 
                    verbose = FALSE, 
                    new.assay.name = "SCT_withULtranscripts") 

# Extract the expression of the UL transcripts
DefaultAssay(data) <- "SCT_withULtranscripts"
...
if (length(remove_genes) > 0) {
  expression_matrix <- GetAssayData(data, 
                                    assay = "SCT_withULtranscripts", 
                                    layer = "data")[remove_genes, , drop = FALSE]
  ...
  data@meta.data <- cbind(data@meta.data, expression_metadata)
} else {
...
data <- SCTransform(data, vst.flavor = "v2", verbose = FALSE, new.assay.name = "SCT") %>%
