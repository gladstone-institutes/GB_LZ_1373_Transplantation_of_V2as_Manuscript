...
for res in "${resolutions[@]}"; do
    ...
    --input $results_dir/04_subset_and_visualize_NoPRV/neurons_smp3-7_grch38_noCB_noDF_NoPRV_data_sct_pca_umap.rds \
    --output $results_dir/05_neuron_subclustering_NoPRV/15PC_${res}res_rmBatchEffect \
...
for res in "${resolutions[@]}"; do
    ...
    Rscript $script_dir/10_02_subclustering_cell_type.R \
    --input $results_dir/04_subset_and_visualize_NoPRV/neurons_smp3-7_grch38_noCB_noDF_NoPRV_data_sct_pca_umap.rds \
    --output $results_dir/05_neuron_subclustering_NoPRV/30PC_${res}res_rmBatchEffect \
