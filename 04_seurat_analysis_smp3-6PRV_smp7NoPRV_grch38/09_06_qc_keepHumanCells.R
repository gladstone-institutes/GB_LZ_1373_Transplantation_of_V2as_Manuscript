...
##    Rscript 09_06_qc_keepHumanCells.R
###############################################################################
...
if(!dir.exists(file.path(outdir, "06_qc_no_CB_no_DF_HumanCells")))
  dir.create(file.path(outdir, "06_qc_no_CB_no_DF_HumanCells"), recursive = TRUE)
setwd(outdir)
...
for(i in 1:length(samples_list)){
  ...
  pdf(paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/",obj.name,"_pre_qc_plot.pdf"))
  ...
  pdf(paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/",obj.name,"_post_qc_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
  ...
  expr <- FetchData(object = this.obj, vars = c("USP9Y","UTY","RPS4Y1","YFP","ChR2(H134R)"), slot = "counts")
  this.obj <- this.obj[, which(x = rowSums(expr) > 0)]
  
  # Visualize Y chr genes
  pdf(paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/",obj.name,"_post_qc_post_YChrGene_filter_genes_violinplot.pdf"))
  print(VlnPlot(this.obj, features = c("USP9Y", "UTY", "RPS4Y1")))
  dev.off()
  
  # Visualize QC metrics post filtering for human cells 
  pdf(paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/",obj.name,"_post_qc_post_YChrGene_filter_QCmetrics_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
  ...
  print(paste0("Metadata for sample ",samples_list[i]," post-QC and Y chromosome gene filtering:"))
  print(head(this.obj))
  ...
  sink(file.path(outdir,"06_qc_no_CB_no_DF_HumanCells/UL_transcripts_post_qc_samples_3-7_grch38_HumanCells.txt"), append = TRUE)
  ...
  saveRDS(this.obj, file = paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/",obj.name,"_HumanCells.rds"))
  
}

#save the qc metadata
write.csv(cells_per_sample, 
          file = paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/qc_per_sample_metadata_samples_3-7_grch38_HumanCells.csv"), 
          row.names = FALSE)

#create a plot for cell counts before and after filtering for each sample
pdf(paste0(outdir, "/06_qc_no_CB_no_DF_HumanCells/","qc_cells_per_sample_plot_samples_3-7_grch38_HumanCells.pdf"),
    width = 20)
print(ggplot(cells_per_sample, 
        ...
        scale_fill_brewer(palette = "Set1",
                          labels = c("Pre-QC", "Post-QC","Post-YChrGene-filter"))+
...
write.csv(as.data.frame(ribo_genes), row.names = F, 
          file = paste0(outdir,"/06_qc_no_CB_no_DF_HumanCells/ribo_genes_samples_3-7_grch38_HumanCells.csv"))
