#!/usr/bin/env Rscript
###############################################################################
## Project ID: GB-LZ-1373
## Authors: Ayushi Agrawal
## Script Goal: Create Seurat object from cell ranger count filtered matrices 
##              samples 3,4,5,6, and 7; perform QC on both samples
## Example running this script on wynton:
##    Rscript 09_14_qc_keep_XRCC5_HumanCells.R
###############################################################################

#load required packages
library(Seurat)
library(dplyr)
library(ggplot2)

#set all directory paths
basedir <- paste0("/gladstone/bioinformatics/projects/lz-1373-lana-zholudeva-",
                  "deepak-srivastava-scrnaseq-snrnaseq-hg38-may-2023/results")
indir <- paste0(basedir,"/01_cellranger_count/")
outdir <- paste0(basedir,"/09_seurat_analysis_smp3-6PRV_smp7NoPRV_grch38_noCB_noDF")
path_postfix_filtered_matrix <- "/outs/filtered_feature_bc_matrix"

#create the output directory
if(!dir.exists(file.path(outdir, "10_qc_noCB_noDF_XRCC5_HumanCells")))
  dir.create(file.path(outdir, "10_qc_noCB_noDF_XRCC5_HumanCells"), recursive = TRUE)
setwd(outdir)


#get the list of all rat/human spinal cord samples
samples_list <- c("LZ4082_03A_human","LZ4082_03B_human",
                  "LZ4082_04A_human","LZ4082_04B_human",
                  "LZ4087_05A_human","LZ4087_05B_human",
                  "LZ4087_06A_human","LZ4087_06B_human",
                  "LZ4087_07A_human_noPRV","LZ4087_07B_human_noPRV")

#create a data frame to record cell count before and after filtering per sample
cells_per_sample <- data.frame(sample=character(), 
                               filter_stage=character(), 
                               number_of_cells=numeric(),
                               nFeature_RNA_min_cutoff=numeric(),
                               nFeature_RNA_max_cutoff=numeric(),
                               percent.mt_cutoff=numeric(), 
                               stringsAsFactors = FALSE)

# list of ribosomal genes in the data
ribo_genes <- NA

#read in data for each sample and perform QC
for(i in 1:length(samples_list)){
  print(paste0("Analyzing sample: ", samples_list[i], "----------------------"))
  obj.name <- samples_list[i]
  datadir <- paste0(indir,samples_list[i],path_postfix_filtered_matrix)
  obj.data <- Read10X(data.dir = datadir)
  this.obj <- CreateSeuratObject(counts = obj.data,
                                 # Don't accept data with fewer than 3 cells
                                 min.cells=3,
                                 # Don't accept data with fewer than 200 genes
                                 min.features=200)
  cells_pre_QC <- ncol(this.obj)
  cells_per_sample[nrow(cells_per_sample)+1,] <- c(obj.name,"pre_QC",
                                                   cells_pre_QC,0,0,0)
  
  #calculate the percentage of mitochondrial genes in each cell
  this.obj[["percent.mt"]] <- PercentageFeatureSet(this.obj, 
                                                   pattern = "^MT-")
  
  #calculate the percentage of ribosomal genes in each cell
  this.obj[["percent.ribo"]] <- PercentageFeatureSet(this.obj, 
                                                     pattern = "^RP[SL][[:digit:]]|^RPLP[[:digit:]]|^RPSA")
  
  
  #update list of ribosomal genes in the data
  if(i==1){
    ribo_genes <- rownames(this.obj)[grep("^RP[SL][[:digit:]]|^RPLP[[:digit:]]|^RPSA", 
                                          rownames(this.obj))]
  }else{
    ribo_genes <- unique(c(ribo_genes, 
                           rownames(this.obj)[grep("^RP[SL][[:digit:]]|^RPLP[[:digit:]]|^RPSA", 
                                                   rownames(this.obj))]))
  }
  
  #Visualize QC metrics
  pdf(paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/",obj.name,"_pre_qc_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
                ncol = 4))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "percent.mt"))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "nFeature_RNA"))
  print(FeatureScatter(this.obj, feature1 = "percent.ribo",
                       feature2 = "percent.mt"))
  dev.off()
  
  #filter based on QC cutoffs
  percent.mt.cutoff <- 0.20
  nfeature.min.cutoff <- 200
  nfeature.max.cutoff <- quantile(this.obj$nFeature_RNA, 0.99)
  this.obj <- subset(this.obj,subset = nFeature_RNA > nfeature.min.cutoff & 
                       nFeature_RNA < nfeature.max.cutoff & 
                       percent.mt < percent.mt.cutoff )
  
  # get number of cells post filtering
  cells_post_QC <- ncol(this.obj)
  cells_per_sample[nrow(cells_per_sample)+1,] <- c(obj.name,
                                                   "post_QC",
                                                   cells_post_QC,
                                                   nfeature.min.cutoff,
                                                   nfeature.max.cutoff,
                                                   percent.mt.cutoff)
  
  # Visualize post QC metrics
  pdf(paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/",obj.name,"_post_qc_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
                ncol = 4))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "percent.mt"))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "nFeature_RNA"))
  print(FeatureScatter(this.obj, feature1 = "percent.ribo",
                       feature2 = "percent.mt"))
  dev.off()
  
  
  # filter for human cells using Y chr genes and marker genes
  expr <- FetchData(object = this.obj, vars = c("XRCC5"), slot = "counts")
  this.obj <- this.obj[, which(x = rowSums(expr) > 0)]
  
  # Visualize Y chr genes
  pdf(paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/",obj.name,"_post_qc_post_XRCC5gene_filter_genes_violinplot.pdf"),
    width = 18)
  print(VlnPlot(this.obj, features = c("USP9Y","UTY","RPS4Y1","YFP","ChR2(H134R)", "XRCC5")))
  dev.off()
  
  # Visualize QC metrics post filtering for human cells 
  pdf(paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/",obj.name,"_post_qc_post_XRCC5gene_filter_QCmetrics_plot.pdf"))
  print(VlnPlot(this.obj, features = c("nFeature_RNA",
                                       "nCount_RNA",
                                       "percent.mt",
                                       "percent.ribo"),
                ncol = 4))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "percent.mt"))
  print(FeatureScatter(this.obj, feature1 = "nCount_RNA",
                       feature2 = "nFeature_RNA"))
  print(FeatureScatter(this.obj, feature1 = "percent.ribo",
                       feature2 = "percent.mt"))
  dev.off()
  
  #add metadata
  this.obj$sample_name <- substr(obj.name,1,9)
  this.obj$library_name <- obj.name
  this.obj$data_type <- "single-nucleus"
  this.obj$species <- "human"
  this.obj$prv_used <- ifelse(this.obj$sample_name == "LZ4087_07",
                              "no", "yes")
  this.obj$library_prep_batch <- ifelse(this.obj$sample_name %in% c("LZ4082_03", "LZ4082_04"),
                                        "batch_1", "batch_2")
  this.obj$experiment <- "new"
  
  print(paste0("Metadata for sample ",samples_list[i]," post-QC and XRCC5 gene filtering:"))
  print(head(this.obj))
  
  #get number of cells post filtering
  cells_post_gene_filter <- ncol(this.obj)
  cells_per_sample[nrow(cells_per_sample)+1,] <- c(obj.name,"post_XRCC5gene_filter",
                                                   cells_post_gene_filter,0,0,0)
  
  
  
  #print the results to a text file
  sink(file.path(outdir,"10_qc_noCB_noDF_XRCC5_HumanCells/UL_transcripts_post_qc_samples_3-7_grch38_XRCC5_HumanCells.txt"), append = TRUE)
  genes_of_interest <- c("UL15","UL18","UL19","UL20","UL21","UL25","mRFP1","YFP","ChR2(H134R)", "USP9Y", "UTY", "RPS4Y1")
  print(paste0("Sample ",samples_list[i], " :"))
  if(sum(genes_of_interest %in% rownames(this.obj)) > 0){
    print(rowSums(this.obj[genes_of_interest,]@assays$RNA@counts))
  } else{
    print("No UL or marker genes found")
  }
  sink()
  
  #save the filtered Seurat object
  saveRDS(this.obj, file = paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/",obj.name,"_XRCC5_HumanCells.rds"))
  
}

#save the qc metadata
write.csv(cells_per_sample, 
          file = paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/qc_per_sample_metadata_samples_3-7_grch38_XRCC5_HumanCells.csv"), 
          row.names = FALSE)

#create a plot for cell counts before and after filtering for each sample
pdf(paste0(outdir, "/10_qc_noCB_noDF_XRCC5_HumanCells/","qc_cells_per_sample_plot_samples_3-7_grch38_XRCC5_HumanCells.pdf"),
    width = 20)
print(ggplot(cells_per_sample, 
             aes(factor(sample), 
                 as.numeric(number_of_cells), 
                 fill = rev(filter_stage))) + 
        geom_bar(stat="identity", 
                 position = "dodge") + 
        scale_fill_brewer(palette = "Set1",
                          labels = c("Pre-QC", "Post-QC","Post-XRCC5gene-filter"))+
        guides(fill=guide_legend(title="QC stage")) +
        xlab("Sample id") +
        ylab("Number of cells") +
        ggtitle("Cells per sample - Pre and Post QC") +
        theme_bw() +
        theme(plot.title = element_text(hjust = 0.5))
) 
dev.off()


#save the list of ribosomal genes
write.csv(as.data.frame(ribo_genes), row.names = F, 
          file = paste0(outdir,"/10_qc_noCB_noDF_XRCC5_HumanCells/ribo_genes_samples_3-7_grch38_XRCC5_HumanCells.csv"))





#save the session info
writeLines(capture.output(sessionInfo()), 
           file.path(outdir, "sessionInfo.txt"))

#record logs
print("***** Script completed! *****")

########################## END ##########################